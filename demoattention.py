...
parser.add_argument('--lr', metavar='LR', default=0.1, type=float,
                    help='learning rate')
parser.add_argument('--nConv', metavar='M', default=1, type=int,
...
class MyNet(nn.Module):
    def __init__(self, input_dim):
        ...
        self.conv1 = SpectralNorm(nn.Conv2d(input_dim, args.nChannel, kernel_size=3, stride=1, padding=1))
        ...
        for i in range(args.nConv - 1):
            self.conv2.append(SpectralNorm(nn.Conv2d(args.nChannel, args.nChannel, kernel_size=3, stride=1, padding=1)))
            self.bn2.append(nn.BatchNorm2d(args.nChannel))
            self.attention2.append(Self_Attn(args.nChannel, 'relu'))
